...
namespace VPKSoft.LangLib
{
    ...
    public partial class DBLangEngine : GuiObjectsEnum
    {
        ...
        public void RunDBCacheMySQL(string app_form, CultureInfo ci)
        {
            ...
            using (MySqlCommand command = new MySqlCommand(SelectDBCache(app_form, ci), MySQLConnection))
            {
        ...
        /// <param name="conn">Reference to a MySqlConnection class instance.</param>
        public static void SaveMessages(string resourcefile, Assembly assembly, ref MySqlConnection conn)
        {
            if (messagesSaved)
            {
                return;
        ...
        public List<string> LocalizedPropsMySQL(string app_form, CultureInfo ci)
        {
            List<string> handled = new List<string>();
            using (MySqlCommand command = new MySqlCommand(SelectLocalizedProps(app_form, ci), MySQLConnection))
            {
                using (MySqlDataReader dr = command.ExecuteReader())
                {
                    while (dr.Read())
        ...
        public void InsertLangItemMySQL(string app_form, string item, string propertyName, string valueType, string value, CultureInfo ci)
        {
            if (buffer)
            {
            ...
            {
                using (MySqlCommand command = new MySqlCommand(InsertLangItem(app_form, item, propertyName, valueType, value, ci), MySQLConnection))
                {
                    command.ExecuteNonQuery();
                }
            }
        ...
        public static string GetStatMessageMySQL(string name, CultureInfo ci, string defaultMessage, params object[] items)
        {
            ...
            MySqlConnection statDBLangConnnection = new MySqlConnection(connStr);
            statDBLangConnnection.Open();
            using (statDBLangConnnection)
            {
                using (MySqlCommand command = new MySqlCommand(SelectStatMessage(ci.Name, name), statDBLangConnnection))
                {
                    using (MySqlDataReader dr = command.ExecuteReader())
                    {
                        if (dr.Read())
        ...
        /// <param name="items">Parameters for formatting the message.</param>
        /// <returns>A message based on the rules in the summary.</returns>
        public string GetMessageMySQL(string name, CultureInfo ci, string defaultMessage, params object[] items)
        {
            ...
            using (MySqlCommand command = new MySqlCommand(SelectMessage(ci.Name, name), MySQLConnection))
            {
                using (MySqlDataReader dr = command.ExecuteReader())
                {
                    if (dr.Read())
        ...
        public List<CultureInfo> GetLocalizedCulturesMySQL()
        {
            List<CultureInfo> retVal = new List<CultureInfo>();
            using (MySqlCommand command = new MySqlCommand("SELECT DISTINCT CULTURE FROM " + Schema + "FORMITEMS ", MySQLConnection))
            {
                using (MySqlDataReader dr = command.ExecuteReader())
                {
                    while (dr.Read())
