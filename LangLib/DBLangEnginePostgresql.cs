...
namespace VPKSoft.LangLib
{
    ...
    public partial class DBLangEngine : GuiObjectsEnum
    {
        ...
        /// <param name="conn">Reference to a NpgsqlConnection class instance.</param>
        public static void SaveMessages(string resourcefile, Assembly assembly, ref NpgsqlConnection conn)
        {
            if (messagesSaved)
            {
                return;
        ...
        public List<string> LocalizedPropsPostgreSQL(string app_form, CultureInfo ci)
        {
            List<string> handled = new List<string>();
            using (NpgsqlCommand command = new NpgsqlCommand(SelectLocalizedProps(app_form, ci), PostgreConnection))
            {
                using (NpgsqlDataReader dr = command.ExecuteReader())
                {
                    while (dr.Read())
        ...
        public void InsertLangItemPostgreSQL(string app_form, string item, string propertyName, string valueType, string value, CultureInfo ci)
        {
            if (buffer)
            {
            ...
            {
                using (NpgsqlCommand command = new NpgsqlCommand(InsertLangItem(app_form, item, propertyName, valueType, value, ci), PostgreConnection))
                {
                    command.ExecuteNonQuery();
                }
            }
        ...
        public static string GetStatMessagePostgreSQL(string name, CultureInfo ci, string defaultMessage, params object[] items)
        {
            ...
            string connStr = dbConnectionStr == string.Empty ? string.Format("Server={0};Port={1};Database={2};Uid={3};Pwd={4};", dbHost, dbPort, dbName, dbUser, dbPassword) : dbConnectionStr;
            NpgsqlConnection statDBLangConnnection = new NpgsqlConnection(connStr);
            statDBLangConnnection.Open();
            using (statDBLangConnnection)
            {
                using (NpgsqlCommand command = new NpgsqlCommand(SelectStatMessage(ci.Name, name), statDBLangConnnection))
                {
                    using (NpgsqlDataReader dr = command.ExecuteReader())
                    {
                        if (dr.Read())
        ...
        /// <param name="items">Parameters for formatting the message.</param>
        /// <returns>A message based on the rules in the summary.</returns>
        public string GetMessagePostgreSQL(string name, CultureInfo ci, string defaultMessage, params object[] items)
        {
            ...
            using (NpgsqlCommand command = new NpgsqlCommand(SelectMessage(ci.Name, name), PostgreConnection))
            {
                using (NpgsqlDataReader dr = command.ExecuteReader())
                {
                    if (dr.Read())
        ...
        public List<CultureInfo> GetLocalizedCulturesPostgreSQL()
        {
            List<CultureInfo> retVal = new List<CultureInfo>();
            using (NpgsqlCommand command = new NpgsqlCommand("SELECT DISTINCT CULTURE FROM " + Schema + "FORMITEMS ", PostgreConnection))
            {
                using (NpgsqlDataReader dr = command.ExecuteReader())
                {
                    while (dr.Read())
